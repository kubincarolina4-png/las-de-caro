...
import {
  ShoppingBag, Plus, Minus, Trash2, X, Search, ChevronRight, ChevronLeft, Loader2,
...
function ProductCard({ product, index, imageCache, onAdd, onOpen, ink }) {
  ...
  return (
    <div
      ...
      className="flex flex-col rounded-xl overflow-hidden cursor-pointer transition-transform active:scale-[0.98]"
      style={{ background: "#FBF7EF", border: "1px solid #E6D9C2" }}
    >
          ...
          <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-1">
            {imgs.map((_, i) => (
              <button
                ...
                style={{ background: i === active ? "#3A3128" : "#3A312855" }}
  ...
  return (
    ...
    >
      <button
        ...
        style={{ background: "#F7F2E7", color: "#3A3128" }}
        aria-label="Cerrar"
      >
        <X size={18} />
      </button>
      ...
      {images.length > 1 && (
        <button
          ...
          style={{ background: "#F7F2E7cc", color: "#3A3128" }}
        ...
        <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-1.5">
          {images.map((_, i) => (
            <span
              key={i}
              className="w-1.5 h-1.5 rounded-full"
              style={{ background: i === index ? "#F7F2E7" : "#F7F2E766" }}
            />
          ))}
        </div>
      )}
...
function ProductDetail({ product, imageCache, onBack, onAdd, onWhatsApp, ink }) {
  const [active, setActive] = useState(0);
  ...
  return (
    <div className="px-5 sm:px-8 py-8 max-w-4xl mx-auto">
      <button
        type="button"
        onClick={onBack}
        className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-wider mb-6"
        style={{ color: "#6B6155" }}
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="flex flex-col gap-2.5">
          <div
            className="aspect-square rounded-xl overflow-hidden relative cursor-zoom-in"
            style={{ background: "#FBF7EF", border: "1px solid #E6D9C2" }}
            ...
            <div className="flex gap-2 flex-wrap">
              {imgs.map((src, i) => (
                <button
                  ...
                  style={{ border: i === active ? "2px solid #3A3128" : "1px solid #E6D9C2" }}
                >
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-wider" style={{ color: "#C98868" }}>
            <span>{product.category}</span>
            {product.color && <span style={{ color: "#8C7F6A" }}>· {product.color}</span>}
          </div>
          <h1 className="font-display italic text-3xl leading-tight">{product.name}</h1>
          <div className="font-display italic text-2xl mt-1">{formatARS(product.price)}</div>
          {product.description && (
            <p className="text-sm leading-relaxed mt-2" style={{ color: "#6B6155" }}>{product.description}</p>
          )}
          ...
          <div className="flex flex-col gap-2.5 mt-6">
            <button
              ...
              className="flex items-center justify-center gap-2 py-3 rounded-full text-sm"
              style={{ background: ink, color: "#F7F2E7" }}
            >
      ...
      {lightboxOpen && imgs.length > 0 && (
        <ImageLightbox images={imgs} startIndex={active} onClose={() => setLightboxOpen(false)} />
      )}
    </div>
  );
}
...
export default function LasDeCaro() {
  ...
  function goToWhatsAppForProduct(product) {
    const digits = (settings.whatsapp || "").replace(/[^\d]/g, "");
    const text = encodeURIComponent(
      `Hola ${settings.storeName}! Quería consultar por: ${product.name} — ${formatARS(product.price)}`
    );
    window.open(digits ? `https://wa.me/${digits}?text=${text}` : `https://wa.me/?text=${text}`, "_blank");
  }
      ...
      <div className="relative" style={{ zIndex: 1 }}>
        ...
        {selectedProduct ? (
          <ProductDetail
            ...
            onBack={closeProduct}
            onAdd={addToCart}
            onWhatsApp={goToWhatsAppForProduct}
            ink={ink}
          />
        ) : (
          <>
        {/* -------------------------- Hero -------------------------- */}
        <section id="inicio" className="px-5 sm:px-8 pt-14 pb-16 max-w-3xl mx-auto text-center">
          <div className="relative inline-block">
            <Sprig className="hidden sm:block absolute -left-16 -top-8 w-14 h-20 opacity-70" />
            ...
            <p className="mt-4 text-base leading-relaxed max-w-md mx-auto" style={{ color: "#6B6155" }}>Objetos hechos con amor para llenar tus espacios de calidez.</p>
        ...
        <section id="productos" ref={productsRef} className="px-5 sm:px-8 py-14 max-w-6xl mx-auto">
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {filtered.map((p, i) => (
                <ProductCard key={p.id} product={p} index={i} imageCache={imageCache} onAdd={addToCart} onOpen={openProduct} ink={ink} />
              ))}
            </div>
          )}
        </section>
